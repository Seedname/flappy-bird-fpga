...
module game (
    ...
    number #(.DIGITS(10), .CHAR_GAP(0), .ALIGN(CENTER_ALIGN)) score_text (
        .num(score), .x(WIDTH / 2), .y(30), .display(state == GAME), .h_count(h_count), .v_count(v_count), .pixel(elements[5])
    );
    ...
    sprite #(.ROM(FLAPPY_BIRD_ROM), .WIDTH(FLAPPY_BIRD_WIDTH), .HEIGHT(FLAPPY_BIRD_HEIGHT)) flappy_bird_logo (
        .x((WIDTH - FLAPPY_BIRD_WIDTH*SCALE)/2), .y((bird_y >>> 3) - BIRD1_HEIGHT*SCALE/2), .display(state == START), .h_count(h_count), .v_count(v_count), .pixel(elements[7])
    );

    // start button
    sprite #(.ROM(START_ROM), .WIDTH(START_WIDTH), .HEIGHT(START_HEIGHT)) start_button (
        .x((WIDTH - START_WIDTH*SCALE)/2), .y(FLOOR_Y - 50), .display(state == START), .h_count(h_count), .v_count(v_count), .pixel(elements[8])
    );
    ...
    sprite #(.ROM(GET_READY_ROM), .WIDTH(GET_READY_WIDTH), .HEIGHT(GET_READY_HEIGHT)) get_ready_logo (
        .x((WIDTH - GET_READY_WIDTH*SCALE)/2), .y(HEIGHT / 4 - GET_READY_HEIGHT*SCALE/2), .display(state == READY), .h_count(h_count), .v_count(v_count), .pixel(elements[9])
    );

    // tap button
    sprite #(.ROM(TAP_ROM), .WIDTH(TAP_WIDTH), .HEIGHT(TAP_HEIGHT)) tap_button (
        .x(WIDTH/2 - 20), .y(HEIGHT/2), .display(state == READY), .h_count(h_count), .v_count(v_count), .pixel(elements[10])
    );


    // game over screen
    sprite #(.ROM(GAME_OVER_ROM), .WIDTH(GAME_OVER_WIDTH), .HEIGHT(GAME_OVER_HEIGHT)) game_over_logo (
        .x((WIDTH - GAME_OVER_WIDTH*SCALE)/2), .y(HEIGHT / 4 - GAME_OVER_HEIGHT*SCALE/2), .display(state == OVER), .h_count(h_count), .v_count(v_count), .pixel(elements[11])
    );

    // scoreboard
    sprite #(.ROM(SCOREBOARD_ROM), .WIDTH(SCOREBOARD_WIDTH), .HEIGHT(SCOREBOARD_HEIGHT)) scoreboard_menu (
        .x((WIDTH - SCOREBOARD_WIDTH*SCALE)/2), .y(HEIGHT/2 - SCOREBOARD_HEIGHT*SCALE/2 + game_over_offset), .display(state == OVER), .h_count(h_count), .v_count(v_count), .pixel(elements[12])
    );

    // new best
    sprite #(.ROM(NEW_ROM), .WIDTH(NEW_WIDTH), .HEIGHT(NEW_HEIGHT)) new_best_text (
        .x((WIDTH - SCOREBOARD_WIDTH*SCALE)/2 + SCOREBOARD_WIDTH*SCALE - NEW_WIDTH*SCALE - 60), .y(HEIGHT/2 - SCOREBOARD_HEIGHT*SCALE/2 + 58 + game_over_offset), .display(state == OVER && new_best), .h_count(h_count), .v_count(v_count), .pixel(elements[13])
    );

    // old score text
    number #(.DIGITS(10), .CHAR_GAP(1), .ALIGN(RIGHT_ALIGN)) old_score_text (
        .num(score), .x((WIDTH - SCOREBOARD_WIDTH*SCALE)/2 + SCOREBOARD_WIDTH*SCALE - 20), .y(HEIGHT/2 - SCOREBOARD_HEIGHT*SCALE/2 + 32 + game_over_offset), .display(state == OVER), .h_count(h_count), .v_count(v_count), .pixel(elements[14])
    );

    // high score text
    number #(.DIGITS(10), .CHAR_GAP(1), .ALIGN(RIGHT_ALIGN)) high_score_text (
        .num(new_best ? score : high_score), .x((WIDTH - SCOREBOARD_WIDTH*SCALE)/2 + SCOREBOARD_WIDTH*SCALE - 20), .y(HEIGHT/2 - SCOREBOARD_HEIGHT*SCALE/2 + 74 + game_over_offset), .display(state == OVER), .h_count(h_count), .v_count(v_count), .pixel(elements[15])
    );
    ...
    medal medal_display (.x(MEDAL_X), .y(MEDAL_Y + game_over_offset), .selected(selected_medal), .display((score_binary >= 10 || high_score_binary >= 10) && state == OVER), .h_count(h_count), .v_count(v_count), .pixel(elements[16]));
    ...
    function logic signed [10:0] random_y(input logic [9:0] seed);
        int range;
        range = MAX_PIPE_Y - MIN_PIPE_Y;
        return (seed * range) / 1024 + MIN_PIPE_Y;
    endfunction
        ...
        if (rst) begin
            ...
            if (state == START || state == READY) begin
                ...
                if (state == START) begin
                    ...
                    // if pressed button 1, continue to ready 
                    if (!button1) begin
                        state <= READY;
                    end
                    ...
                    if (!button0) begin
                        if (!jumped_last) begin
                            pipe_y <= '{random_y(next_random[9:0]), random_y(next_random[19:10]), random_y(next_random[29:20])};
                            state <= GAME;
                            bird_vy <= -BIRD_SPEED;
                        end
                        jumped_last <= 1;
            ...
            if (!dead) begin
                ...
                if (state == GAME) begin
                    ...
                    if (pipe_x[cp] + PIPE_BOTTOM_WIDTH * SCALE <= START_X) begin
                        ...
                        pipe_y[cp] <= random_y(next_random[9:0]);
                        current_pipe <= (cp + 1) % 3;
                        refresh_score <= 0;
                    end
                end
